      ...
      <!-- Main Content Grid -->
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <form @submit.prevent="submit" class="space-y-6">
            ...
            <!-- Team Leader Section -->
            <div class="bg-white rounded-2xl shadow-lg border-2 border-indigo-100 overflow-hidden hover:shadow-xl transition-shadow duration-300">
              ...
              <div class="p-6">
                ...
                <div class="relative">
                  <select v-model="form.team_leader_id" 
                          class="w-full px-4 py-3 pr-10 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-indigo-100 focus:border-indigo-400 transition-all duration-200 appearance-none bg-white text-gray-800 font-medium">
                  ...
                  <div v-for="(it, i) in form.items" :key="i" 
                    ...
                    <div class="flex items-center justify-between mb-4">
                      ...
                      <button v-if="!usaTamanos && separarItems && form.items.length > 1" 
                              type="button" 
                              @click="removeItem(i)"
                              class="px-3 py-1.5 text-sm font-medium text-red-600 hover:text-white bg-red-50 hover:bg-red-600 rounded-lg transition-all duration-200 flex items-center gap-1">
                    ...
                    <div v-if="usaTamanos" class="grid md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <div class="px-4 py-3 bg-gray-100 border-2 border-gray-300 rounded-xl text-gray-700 font-semibold uppercase">
                          {{ it.tamano || 'N/A' }}
                        </div>
                        <p class="mt-1 text-xs text-gray-500">Campo bloqueado (viene de la solicitud)</p>
                      </div>
                      <div>
                        ...
                        <p class="mt-1 text-xs text-gray-500">Campo bloqueado (cantidad aprobada)</p>
                      </div>
                    </div>
                    ...
                    <div v-else class="space-y-4">
                      <div>
                        ...
                        <textarea v-model="it.descripcion" 
                                  rows="2"
                                  class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-emerald-100 focus:border-emerald-400 transition-all duration-200 resize-none"
                                  placeholder="Ej: Lenovo ThinkPad T480, Asus VivoBook 15, Dell Latitude 5400, etc."></textarea>
                        <p class="mt-1 text-xs text-gray-500">Describe el trabajo o producto específico</p>
                      </div>
                      <div>
                        ...
                        <div class="relative">
                          <input type="number" 
                                 ...
                                 class="w-full px-4 py-3 pl-10 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-emerald-100 focus:border-emerald-400 transition-all duration-200" 
                                 placeholder="0" />
                        ...
                        <p class="mt-1 text-xs text-gray-500">Cantidad de este ítem específico</p>
                      </div>
                    </div>
                  </div>
                </div>
    ...
    <button v-if="!usaTamanos" 
            ...
            <div class="bg-white rounded-2xl shadow-lg border-2 border-upper-50 overflow-hidden">
              <div class="p-6">
                ...
                <div v-if="!usaTamanos && separarItems && alertaSuma !== 'success' && !form.processing" class="mt-3 bg-yellow-50 border-2 border-yellow-200 rounded-xl p-3">
            ...
            <div v-if="$page.props?.flash?.ok" 
                 class="bg-gradient-to-r from-emerald-50 to-teal-50 border-2 border-emerald-300 rounded-2xl p-5 flex items-center gap-3">
              <div class="bg-emerald-500 p-2 rounded-full">
                <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                </svg>
              </div>
              <p class="text-emerald-800 font-semibold">{{ $page.props.flash.ok }}</p>
            </div>
          </form>
        </div>
        ...
        <div class="lg:col-span-1">
          <div v-if="cotizacion?.lines?.length" class="bg-white rounded-2xl shadow-lg border-2 border-orange-100 overflow-hidden sticky top-6 hover:shadow-xl transition-shadow duration-300">
