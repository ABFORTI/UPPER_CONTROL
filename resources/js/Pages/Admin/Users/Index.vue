...
<template>
  ...
  <div class="bg-gradient-to-r from-indigo-600 to-[#1E1C8F] rounded-2xl shadow-xl p-6 sm:p-8 mb-6">
       ...
       <a :href="route('admin.users.create')" 
         class="inline-flex items-center justify-center gap-2 px-6 py-3 bg-white text-[#1E1C8F] font-bold rounded-xl hover:bg-indigo-50 transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105 w-full sm:w-auto">
      ...
      <div class="bg-white rounded-2xl shadow-lg border-2 border-gray-100 overflow-hidden">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr v-for="u in data.data" :key="u.id" class="hover:bg-indigo-50 transition-colors duration-150">
                <td class="px-4 lg:px-6 py-3">
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div class="text-xs sm:text-sm text-gray-500">{{ u.email }}</div>
                    </div>
                  </div>
                </td>
                <td class="px-4 lg:px-6 py-3 text-gray-600">{{ u.centro_nombre || '—' }}</td>
                <td class="px-4 lg:px-6 py-3">
                  <div v-if="u.centros_nombres?.length" class="flex flex-wrap gap-1">
                    <span v-for="(centro, idx) in u.centros_nombres" :key="idx"
                          class="px-2 py-1 bg-indigo-100 text-indigo-700 rounded-full text-[0.65rem] sm:text-xs font-medium">
                      {{ centro }}
                    </span>
                  </div>
                ...
                <td class="px-4 lg:px-6 py-3 text-center">
                  ...
                  <span v-else class="inline-flex items-center gap-1 px-3 py-1 bg-red-100 text-red-700 rounded-full text-xs font-semibold">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                ...
                <td class="px-4 lg:px-6 py-3">
                  <div class="flex flex-wrap items-center justify-end gap-2">
                    <a :href="route('admin.users.edit', u.id)" 
                       class="inline-flex items-center gap-1 px-3 sm:px-4 py-2 bg-[#1E1C8F] text-white text-xs sm:text-sm font-medium rounded-lg hover:bg-indigo-800 transition-colors duration-200">
                    ...
                    <button @click="toggle(u.id)" 
                            :class="[
                              'inline-flex items-center gap-1 px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium rounded-lg transition-colors duration-200',
                    ...
                    <button @click="impersonate(u.urls.impersonate)" 
                            class="inline-flex items-center gap-1 px-3 sm:px-4 py-2 bg-amber-100 text-amber-700 text-xs sm:text-sm font-medium rounded-lg hover:bg-amber-200 transition-colors duration-200">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="md:hidden p-4 space-y-4">
          <div v-for="u in data.data" :key="`m-${u.id}`" class="border border-gray-200 rounded-2xl p-4 shadow-sm bg-white">
            <div class="flex items-start gap-3">
              <div class="flex-shrink-0 h-10 w-10 bg-[#1E1C8F] rounded-full flex items-center justify-center text-white font-bold">
                {{ u.name?.charAt(0)?.toUpperCase() || '?' }}
              </div>
              <div class="flex-1">
                <div class="text-lg font-semibold text-gray-900">{{ u.name }}</div>
                <div class="text-sm text-gray-500 break-all">{{ u.email }}</div>
              </div>
            </div>

            <div class="mt-3 space-y-2 text-sm text-gray-600">
              <div>
                <span class="font-semibold text-gray-700">Centro principal:</span>
                <span class="ml-1">{{ u.centro_nombre || '—' }}</span>
              </div>
              <div>
                <span class="font-semibold text-gray-700">Centros asignados:</span>
                <div class="mt-1 flex flex-wrap gap-1">
                  <span v-if="u.centros_nombres?.length" v-for="(centro, idx) in u.centros_nombres" :key="`c-${idx}`" class="px-2 py-1 bg-indigo-100 text-indigo-700 rounded-full text-xs font-medium">
                    {{ centro }}
                  </span>
              ...
              <div>
                ...
                <span v-if="u.activo" class="ml-2 px-3 py-1 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-700 border border-emerald-200 inline-flex items-center gap-2">
                  <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                  </svg>
                  Activo
                </span>
                <span v-else class="ml-2 px-3 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-700 border border-red-200 inline-flex items-center gap-2">
                  <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"/>
                  </svg>
                  Inactivo
            ...
            <div class="mt-4 flex flex-col sm:flex-row gap-2">
              <a :href="route('admin.users.edit', u.id)"
                 class="flex-1 px-4 py-2 rounded-lg bg-[#1E1C8F] text-white font-semibold hover:bg-indigo-800 transition-colors duration-200 flex items-center justify-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                </svg>
                Editar
              ...
              <button @click="impersonate(u.urls.impersonate)"
                      class="flex-1 px-4 py-2 rounded-lg bg-amber-100 text-amber-700 font-semibold hover:bg-amber-200 transition-colors duration-200 flex items-center justify-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/>
                </svg>
                Impersonar
