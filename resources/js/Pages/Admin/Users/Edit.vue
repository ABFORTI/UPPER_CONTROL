  ...
  <div class="bg-gradient-to-r from-indigo-600 to-[#1E1C8F] rounded-2xl shadow-xl p-8 mb-6">
        <div class="flex items-center gap-4">
          ...
          <div>
            ...
            <p class="text-indigo-100 mt-1">{{ props.user ? 'Actualiza la información del usuario' : 'Crea una nueva cuenta de usuario' }}</p>
          </div>
        </div>
      </div>
  ...
  <div v-if="['admin','calidad','facturacion','control','comercial'].includes(form.role)" 
         class="bg-gradient-to-r from-indigo-50 to-blue-50 rounded-xl p-6 border-2 border-indigo-200">
              ...
              <select v-model="form.centros_ids" 
                      class="w-full px-4 py-3 border-2 border-indigo-200 rounded-xl focus:ring-4 focus:ring-indigo-100 focus:border-indigo-400 transition-all duration-200 bg-white" 
                      multiple 
                      size="6">
              ...
              <p class="text-indigo-700 text-sm mt-3 flex items-center gap-2 font-medium">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                </svg>
                Mantén presionado <kbd class="px-2 py-1 bg-white rounded border border-indigo-300 text-xs">Ctrl</kbd> (Windows) o <kbd class="px-2 py-1 bg-white rounded border border-indigo-300 text-xs">⌘ Cmd</kbd> (Mac) para seleccionar múltiples centros.
              </p>
    ...
    <button @click="save" 
      ...
      class="px-8 py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-[#1E1C8F] font-bold text-white hover:shadow-xl transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed transform hover:scale-105 flex items-center gap-2">
